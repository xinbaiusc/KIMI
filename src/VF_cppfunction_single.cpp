...
void loadToVectorNoPair(int k, unsigned long total, vector<double>& kmerCount)
{
	...
	for(unsigned long currentKmerTen = 0; currentKmerTen < pow(ZI, k); currentKmerTen++)
	{
      ...
      kmerCount.push_back(HashTable[currentKmerTen]/double(total));
			//countWord ++;
...
//	}
	...
	loadToVectorNoPair(k, total, kmerCount);
	//Rcout << "\n total:" << total << endl;
